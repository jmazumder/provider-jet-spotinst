...
import (
	...
	v1alpha1 "github.com/jmazumder/provider-jet-spotinst/apis/managed/v1alpha1"
)

// Setup adds a controller that reconciles InstanceAws managed resources.
func Setup(mgr ctrl.Manager, l logging.Logger, rl workqueue.RateLimiter, s terraform.SetupFn, ws *terraform.WorkspaceStore, cfg *tjconfig.Provider, concurrency int) error {
	name := managed.ControllerName(v1alpha1.InstanceAws_GroupVersionKind.String())
	r := managed.NewReconciler(mgr,
		xpresource.ManagedKind(v1alpha1.InstanceAws_GroupVersionKind),
		managed.WithExternalConnecter(tjcontroller.NewConnector(mgr.GetClient(), ws, s, cfg.Resources["spotinst_managed_instance_aws"])),
